...
import { extractNamespace, isExportData, isLegacyVersion } from '@/background/database/common/migratable'
import { StorageHolder } from '@/background/database/common/storage-holder'
import type { BrowserMigratable, StorageMigratable } from '@/background/database/types'
...
class StatDatabaseWrapper implements StateDatabaseComposite {
    ...
    deleteByGroup(param: timer.stat.SiteDeleteByGroup): Promise<void> {
        return this.current().deleteByGroup(param)
    }
