...
type IncreaseResult = {
    limited: tt4b.limit.Item[]
    reminder?: tt4b.limit.ReminderInfo
}
...
export async function addLimitFocusTime(host: string, url: string, focusTime: number): Promise<IncreaseResult> {
    ...
    const limited: tt4b.limit.Item[] = []
    const needReminder: tt4b.limit.Item[] = []
...
export function calcTimeState(item: tt4b.limit.Item, reminderMills: number, delayDuration: number): LimitTimeStateResult {
...
function addFocusForEach(item: tt4b.limit.Item, focusTime: number, durationMill: number, delayDuration: number): [met: boolean, reminder: boolean] {
...
export async function updateLimitRules(rules: tt4b.limit.Rule[]): Promise<void> {
    await db.batchUpdate(rules)
    await noticeLimitChanged()
}

export async function createLimitRule(rule: Omit<tt4b.limit.Rule, 'id'>): Promise<number> {
