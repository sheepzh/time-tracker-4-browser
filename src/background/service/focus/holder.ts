...
async function abort(record: tt4b.focus.Session): Promise<void> {
    ...
    record.end = now
    record.logs.push({ action: 'abort', ts: now, phase })
...
function calcTotalFocus(logs: tt4b.focus.Session['logs']) {
    ...
    for (const { action, ts, phase } of logs) {
        if (phase !== 'focus') {
            ...
            openTs = ts
        } else if (action === 'finish' || action === 'pause' || action === 'abort') {
...
class FocusHolder {
    ...
    async delay(): Promise<void> {
        if (!this.#session) return
        if (this.#session.template !== 'focus') return
