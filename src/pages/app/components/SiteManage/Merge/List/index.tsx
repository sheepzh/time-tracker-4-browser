...
import Item, { type ItemInstance } from './Item'
...
const _default = defineComponent<{}>(() => {
    ...
    const handleDelete = useOperation(async (origin: string) => {
        await ElMessageBox.confirm(t(msg => msg.siteManage.merge.removeConfirmMsg, { origin }))
    ...
    const handleChange = useOperation(async (origin: string, merged: string | number, index: number) => {
        ...
        if (hasDuplicate) {
            ElMessage.warning(t(msg => msg.siteManage.merge.duplicateMsg, { origin }))
    ...
    const handleAdd = async (origin: string, merged: string | number): Promise<boolean> => {
        ...
        const content = t(msg => msg.siteManage.merge.addConfirmMsg, { origin })
        try {
