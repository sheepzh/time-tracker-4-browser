...
const ComparisonIcon = defineComponent<{ value: RingValue }>(props => {
    const comp = computed(() => {
        const [current = 0, last = 0] = props.value
        if (current === last) return { visible: false }
        const direction = current > last ? 'increase' : 'decrease'
        let count = 0
        if (current === 0 || last === 0) {
            count = 3
        } else {
        ...
        return count ? { visible: true, direction, count } : { visible: false }
    })
    return () => (
        comp.value.visible && <Flex color={`var(--timer-chart-${comp.value.direction}-color)`}>
            {range(comp.value.count ?? 0).map(() => (
                <ElIcon>
                    {comp.value.direction === 'increase' ? <TopRight /> : <BottomRight />}
                </ElIcon>
            ))}
        </Flex>
    )
