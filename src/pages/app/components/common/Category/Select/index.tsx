...
const useCategorySelect = (props: Props) => {
    ...
    // Delete category
    const { refreshAsync: removeCate } = useManualRequest(
        (id: number) => sendMsg2Runtime('cate.delete', id),
        {
            onSuccess: () => {
                cate.refresh()
    ...
    const handleDeleteCategory = async (category: timer.site.Cate, e: MouseEvent) => {
        ...
        // Check related sites
        const sites = await listSites({ cateIds: category.id })
    ...
    const handleEditCategory = async (category: timer.site.Cate, e: MouseEvent) => {
        ...
        try {
            ...
            await sendMsg2Runtime('cate.change', edited)
            cate.refresh()
        } catch {
            // User cancelled
        } finally {
